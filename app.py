...
# --- TAB 2: KALKULATOR PREDIKSI HARGA SEWA ---
with tab2:
    st.header("Kalkulator 2: Memprediksi Harga Sewa ATM")
    st.write(
        "Gunakan jika Anda ingin mencari rekomendasi tarif harga sewa space ATM yang ideal."
    )
    ...
    with col3:
        ...
        l_bangunan_total = st.number_input(
            "Luas Bangunan Total Gedung Induk (m²)", value=120, step=10, key="lb2"
        )
    ...
    st.write(
        f"• Nilai Bangunan Proporsional (Setelah Depresiasi): **Rp {v_bangunan_atm:,.2f}**"
    )
